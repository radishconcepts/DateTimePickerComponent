 ...
 * @desc This module contains the DateRangePicker class
 */
...
export function DateRangePicker(start_id, end_id, settings = {}) {
    ...
    this.date_output = settings.date_output
        ? settings.date_output
        : "short_ISO";
    this.min_range = settings.min_range_hours
        ? settings.min_range_hours * 60 * 60 * 1000
        : 1 * 60 * 60 * 1000;
    ...
    const first_day_no =
        typeof settings.first_day_no !== "undefined"
            ? settings.first_day_no
            : 0;
    const styles = settings.styles ? settings.styles : {};
    this.setStartPickerProps(
        start_id,
        start_date,
        first_date,
        last_date,
    ...
    this.start_container.insertAdjacentHTML(
        "afterbegin",
        this.getHTML("start", "date", styles)
    );
